...
int Socket_Peer_Connect(int __fd, struct sockaddr * __addr, socklen_t __len) {
...
int Bind_Socket(int __fd, struct sockaddr * __addr, socklen_t __len) {
...
int Inet_pton(int __af, const char *__restrict __cp, void *__restrict __buf) {
    if(inet_pton(__af, __cp, __buf) < 0 ) {
        ...
        LOGE("set presentation to numeric error: %s \n", strerror(errno));
       // exit(0);
    }
    return 0;
}
